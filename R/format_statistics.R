...
format_statistics.statistics_numeric <- function(x, ...) {
  ...
  values <- c(Mean_SD = paste0(the_mean, " (", the_sd, ")"),
              valid_missing = paste0(x$length - x$missing, " (", x$missing, ")"))

  format_statistics_out <- data.frame(
    tag = factor(names(values), levels = names(values)),
    value = values, row.names = NULL,
...
format_statistics.statistics_factor <- function(x, ...) {
    ...
    # greatest of them will have a lot of digits. sapply ensures that all
    # numbers have the same number of digits, irrespective of the other numbers.
    value <- paste0(percent, "% (", value, ")")

    format_statistics_out <- data.frame(
      tag = factor(nn, levels = nn),
      value = value,
...
format_statistics.statistics_count_me <- function(x, ...) {
  return(data.frame(tag = factor(""),
                    value = as.character(x[[atable_options("colname_for_observations")]]),
                    stringsAsFactors = FALSE)
  )
}
