...
Sometimes reports in languages other than English are needed. atable's output can be localised. We will set the language to German via the settings package \cite{settings2015}:
...
This new function currently lives in the user's workspace. But it must also live in atable's namespace and replace the already existing method. This is done by:
...
Now replace \verb|statistics.numeric| with a new function which calculates the median, MAD, mean and sd. See the documentation of \verb|statistics|: the function has one argument called $x$ and the ellipsis $\dots$. The function must return a named list.
...
Apply atable to \verb|test_data|, a data.frame with simulated data included in the atable package. Then print the results:
